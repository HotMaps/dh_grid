...
def summary(covered_demand, dist_inv, dist_spec_cost, trans_inv,
    ...
    summary = [{"unit": "MWh", "name": "Total demand in selected region in the first year of investment", "value": float(heat_dem_1st)},
               ...
               {"unit": "EUR", "name": "Total grid costs - NPV", "value": float(dist_inv + trans_inv)},
               {"unit": "EUR", "name": "Total distribution grid costs - NPV", "value": float(dist_inv)},
               {"unit": "EUR", "name": "Total transmission grid costs - NPV", "value": float(trans_inv)},
